...
// ==========================================
// --- Logic điều khiển Màn hình World Cup ---
document.addEventListener('DOMContentLoaded', () => {
    const openBtn = document.getElementById('btn-open-wc');
    const closeBtn = document.getElementById('btn-close-wc');
    ...
    openBtn.addEventListener('click', () => {
        // 1. Hiện panel (hiện invisible trước để animation mượt)
        panel.classList.remove('invisible');
        // 2. Kích hoạt animation trượt vào
        setTimeout(() => {
            panel.classList.remove('translate-x-full');
        }, 10); // delay nhỏ để trình duyệt kịp nhận diện invisible đã mất

        // 3. Khóa cuộn trang lịch âm ở phía dưới
        document.body.classList.add('overflow-hidden');
    });

    // Hàm Đóng Panel
    closeBtn.addEventListener('click', () => {
        // 1. Kích hoạt animation trượt ra
        panel.classList.add('translate-x-full');

        // 2. Ẩn invisible sau khi animation kết thúc (300ms)
        setTimeout(() => {
            panel.classList.add('invisible');
        }, 300); // Khớp với duration-300 ở HTML

        // 3. Mở lại cuộn trang lịch âm
        document.body.classList.remove('overflow-hidden');
    });
    // ==========================================
    // LOGIC ĐIỀU HƯỚNG MENU WORLD CUP
    // ==========================================
    const navBtns = document.querySelectorAll('.wc-nav-btn');
    const scrollContainer = document.getElementById('wc-main-scroll');

    if (navBtns.length > 0 && scrollContainer) {
        navBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                // 1. Lấy ID của section đích từ data-target
                const targetId = btn.getAttribute('data-target');
                const targetSection = document.getElementById(targetId);

                // 2. Cuộn mượt đến section đó
                if (targetSection) {
                    // Tính tổng offsetTop từ section lên đến scrollContainer
                    let scrollPos = 0;
                    let el = targetSection;
                    while (el && el !== scrollContainer) {
                        scrollPos += el.offsetTop;
                        el = el.parentElement;
                    }
                    
                    // Trừ đi chiều cao nav sticky (65px) + scroll-mt-20 (80px)
                    scrollPos -= (65 + 80);
                ...
                // 3. Đổi màu nút đang được chọn (Active state)
                navBtns.forEach(b => {
        ...
        // 4. (Nâng cao) Tự động đổi màu Menu khi cuộn tay qua các section
        const sections = document.querySelectorAll('main#wc-main-scroll section');
        scrollContainer.addEventListener('scroll', () => {
            let current = '';
            sections.forEach(section => {
                const sectionTop = section.offsetTop;
                // Khi cuộn qua cách top 100px thì xác định là đang ở section đó
            ...
            navBtns.forEach(btn => {
                ...
                if (btn.getAttribute('data-target') === current) {
                    ...
                    // Tự động cuộn thanh menu ngang để nút active luôn hiển thị
